...
def tail_file(path: str, n: int = 80) -> List[str]:
    """Reads the last n lines of a file (returns an error message if failed)."""
    try:
...
def spawn_task(script_path: str, task_id: str, cwd: str = CATKIN_WS):
    """
    Starts the script:
    - Uses bash -lc to ensure 'login shell' semantics (more stable under systemd)
    - stdout/stderr are written to LOG_DIR
    """
    ts = time.strftime("%Y%m%d_%H%M%S")
    log_path = str(LOG_DIR / f"{Path(script_path).stem}_{task_id}_{ts}.log")

    # Execute with bash -lc to avoid PATH inconsistencies in systemd environments
    cmd = [
        "bash", "-lc",
        f"bash {shlex.quote(script_path)} {shlex.quote(task_id)}"
    ]

    # Critical: Do not PIPE! Write directly to log file (prevents hanging, allows output viewing)
    logf = open(log_path, "a", buffering=1)  # Line buffering
    env = os.environ.copy()
    ...
    p = subprocess.Popen(
        ...
        cwd=cwd,
        env=env,
        start_new_session=True,   # Process group isolation for easier management/killing later
    )

    # Parent process can close the handle; child process has inherited the FD and can continue writing to the log
    try:
        logf.flush()
        logf.close()
    except Exception:
...
class RobotStateManager:
    ...
    def update_state(self):
        ...
        # Publish ROS status (if available)
...
async def start_task(task: TaskRequest):
    ...
    # 1) Do not allow concurrent tasks
    ...
    # 3) Start child process (Critical: Write logs to disk + fixed cwd)
    try:
...
async def get_status(task_id: Optional[str] = None, log_tail_lines: int = DEFAULT_LOG_TAIL_LINES):
    # Refresh state on query
    state_manager.update_state()

    # 1) Query specific task_id
    if task_id:
